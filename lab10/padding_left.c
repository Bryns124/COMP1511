...
void pad_left(struct character_node *characters, char pad_character) {
    struct character_node *curr_node = characters;
    if (curr_node == NULL) {
        return;
    }
    int tmp_data1 = curr_node->data;
    int tmp_data2;
    curr_node->data = pad_character;
        while (curr_node->next != NULL) {
            curr_node = curr_node->next;
            tmp_data2 = curr_node->data;
            curr_node->data = tmp_data1;
            curr_node = curr_node->next;
            tmp_data1 = curr_node->data;
            curr_node->data = tmp_data2;
        }
}
