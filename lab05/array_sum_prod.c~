...
int main(void) {
    ...
    // the parameters are aligned with the variables inside the definition of the function
...
void array_sum_prod(int length, int nums[length], int *sum, int *product) {
    // we must define the pointer values (these are not the original variables)
    //MUST BE DEFINED INSIDE THE FUNCTIOn 
    *sum = 0;
    *product = 1;
    int x = 0;
    while (x < length) {
